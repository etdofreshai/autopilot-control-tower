...
function renderDashboard() {
  ...
  const running = Boolean(state.snapshot?.loop?.autopilot?.enabled);
  $('#dashboard').innerHTML = `
    <section class="card hero">
      <div>
        <p class="eyebrow">${esc(p.repoPath)}</p>
        <h2>${esc(p.name)}</h2>
        <p class="muted">One real OpenClaw loop. Set the request, then turn it on or off.</p>
      </div>
      <div class="toolbar">
        <button class="${running ? 'danger' : ''}" data-toggle-loop>${running ? 'Stop loop' : 'Start loop'}</button>
      </div>
    </section>
    <div class="tabs">${['loop','history','files'].map(t => `<button class="tab ${state.tab===t?'active':''}" data-tab="${t}">${t}</button>`).join('')}</div>
    <div id="tabBody">${renderTab()}</div>`;
  document.querySelector('[data-toggle-loop]').onclick = running ? stopBackgroundLoop : startBackgroundLoop;
...
function renderLoop() {
  ...
  const running = Boolean(l.autopilot?.enabled);
  const activeRun = (l.agentRuns || []).find(r => r.status === 'running');
  return `
    <section class="grid metrics">
      <div class="metric"><b class="${running ? 'good' : 'warn'}">${running ? 'running' : 'stopped'}</b><span>loop status</span></div>
      <div class="metric"><b>${activeRun ? 'yes' : 'no'}</b><span>agent active now</span></div>
      <div class="metric"><b>${esc(l.model)}</b><span>model</span></div>
      <div class="metric"><b>${esc(l.autopilot?.intervalSeconds || 300)}s</b><span>interval</span></div>
    </section>
    <section class="card">
      <h2>Loop control</h2>
      <form id="intentForm" class="config-grid">
        <label class="wide">Request / intent<textarea name="intent" rows="5" placeholder="What should the OpenClaw loop keep working on?">${esc(l.intent)}</textarea></label>
...
function agentRuns(runs=[]) {
  if (!runs.length) return '<p class="muted">No OpenClaw runs yet. Start the loop to launch real runs from the current request.</p>';
